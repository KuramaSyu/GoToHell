...
interface Friend {
  id: number;
  // adjust these properties as needed depending on your backend response structure
  friendName: string;
  status: string;
}
...
export const FriendOverview: React.FC = () => {
  ...
  const handleUpdateStatus = async (id: number, newStatus: string) => {
    try {
      ...
      setError('Error updating friendship');
    }
  };
          ...
          {loading ? (
            ...
            <List>
              {filteredFriends.map((friend) => (
                <ListItem key={friend.id} sx={{ borderBottom: '1px solid #ccc' }}>
                  <ListItemText
